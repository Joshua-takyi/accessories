...
export default function SearchComponent() {
	...
	const saveSearch = (query: string) => {
		const now = Date.now();
		// Filter out expired searches and the current query if it exists
		const validSearches = recentSearches
			.filter((s) => now - s.timestamp < 3600000) // Remove searches older than 1 hour
			.filter((s) => s.query !== query);

		const newSearches = [{ query, timestamp: now }, ...validSearches].slice(
			0,
			5
	...
	const handleSearch = (query: string) => {
		...
		router.push(
			`/search?type=${encodeURIComponent(
				query
			)}&available=true&sortBy=price&order=asc&page=1`
		);
	};
									...
									>
										<div className="flex gap-3 items-center">
											<div className="relative flex-1 group">
												<Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5 transition-colors duration-150 group-focus-within:text-[#e2780c]" />
												<input
													ref={inputRef}
													type="search"
													value={searchQuery}
